...
本项目代码和 `downloads/` 资源可以放在同一块移动硬盘上跨系统共用，但 **Python 虚拟环境必须按操作系统分开**：

| 系统 | 虚拟环境目录 | Python 路径 | 用途 |
|------|--------------|-------------|------|
| Windows | `venv/` | `venv\Scripts\python.exe` | 原 Windows 开发/下载环境 |
| macOS | `.venv-macos/` | `.venv-macos/bin/python` | Mac 本地启动离线场景 |

**不要在 macOS 上直接复用或覆盖现有 `venv/`**：当前 `venv/` 是 Windows 虚拟环境，里面是 `.exe` 和 `win_amd64.pyd`，macOS 无法执行。若 macOS 需要运行，先在 Mac 本机安装 Python 3.12+，再在项目目录创建独立环境：

```bash
cd "/Volumes/9100 Pro/matterport-dl"
...
之后 macOS 启动模型建议直接调用 `.venv-macos`，避免 `run.py` 自动检测到 Windows `venv/` 后混用目录：

```bash
cd "/Volumes/9100 Pro/matterport-dl"
.venv-macos/bin/python matterport-dl.py Kfa2BHFroVn 127.0.0.1 8081
```

注意：Python 3.12+ 建议安装在 Mac 本机（如 `/opt/homebrew/bin/python3.12`），项目和大型 `downloads/` 资源在 `9100 Pro` 移动硬盘上。未挂载该硬盘时，即使 Mac 本机有 Python，也无法启动这些离线场景。
