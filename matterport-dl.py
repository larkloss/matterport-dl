...
async def downloadFile(type, shouldExist, url, file, post_data=None, always_download=False, key_type: AccessKeyType = AccessKeyType.PrimaryKey):
    ...
    async with MAX_TASKS_SEMAPHORE:
        ...
        try:
            ...
            # Matterport returns HTTP 404 for some private/licensed models even when
            # the response body is a fully valid showcase page. Browsers ignore the
            # status and render it; we do the same if the body contains the expected
            # MP_PREFETCHED_MODELDATA payload.
            if response.status_code == 404 and type == "MAIN" and b"MP_PREFETCHED_MODELDATA" in response.content:
                consoleLog(f"Accepting 404 response for MAIN page (body has MP_PREFETCHED_MODELDATA): {url}")
            else:
...
async def downloadPlugins(groups, allowedPolicies):
    ...
    for plugin in pluginJson:
        ...
        try:
            ...
            consoleDebugLog(f"Plugin {plugin['name']} v{currentVersion} fetch failed (Matterport CDN may have purged this version), skipping: {e}", loglevel=logging.WARNING)
            continue
...
def patchShowcase():
    ...
    # Patch init.js: rename .onStale to break stale callback assignment
    init_path = "js/init.js"
    if os.path.exists(init_path):
        with open(init_path, "r", encoding="UTF-8") as f:
            init_content = f.read()
        init_content = init_content.replace(".onStale", ".onStal")
        with open(getModifiedName(init_path), "w", encoding="UTF-8") as f:
            f.write(init_content)

    # Patch packages-cwf-util.js: disable the stale check entirely
    # Replacing .onStale→.onStal doesn't work here because both setter and getter
    # get renamed consistently, so the logic still runs and blocks with await.
    # Instead, replace the condition to always be false.
    cwf_util_path = "js/packages-cwf-util.js"
    if os.path.exists(cwf_util_path):
        with open(cwf_util_path, "r", encoding="UTF-8") as f:
            cwf_content = f.read()
        cwf_content = cwf_content.replace("this.validUntil&&this.onStale", "this.validUntil&&false")
        with open(getModifiedName(cwf_util_path), "w", encoding="UTF-8") as f:
            f.write(cwf_content)
...
async def downloadCapture(pageid):
    ...
    extra_params = os.environ.get("MATTERPORT_EXTRA_PARAMS", "")
    if extra_params and not extra_params.startswith("&"):
        extra_params = "&" + extra_params
    url = f"https://my.{BASE_MATTERPORT_DOMAIN}/show/?m={pageid}{extra_params}"
...
class OurSimpleHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GraphRequest(self, option_name: str):
        ...
        if option_name in GRAPH_DATA_REQ:
            ...
            logLevel = logging.WARNING
            post_msg = f"graph for operationName: {option_name} is not in GRAPH_DATA_REQ, returning empty JSON"
...
def SetupSession(use_proxy):
    ...
    referer = os.environ.get("MATTERPORT_REFERER", f"https://my.{BASE_MATTERPORT_DOMAIN}/")
    base_headers = {"Referer": referer, "x-matterport-application-name": "showcase"}
    # Optional OAuth bearer for private/licensed models. Pass raw token; "Bearer " prefix added automatically.
    auth_token = os.environ.get("MATTERPORT_AUTH_TOKEN", "")
    if auth_token:
        if not auth_token.lower().startswith("bearer "):
            auth_token = f"Bearer {auth_token}"
        base_headers["Authorization"] = auth_token
    app_key = os.environ.get("MATTERPORT_APPLICATION_KEY", "")
    if app_key:
        base_headers["x-matterport-application-key"] = app_key
    OUR_SESSION = requests.AsyncSession(impersonate="chrome", max_clients=MAX_CONCURRENT_REQUESTS, verify=CLA.getCommandLineArg(CommandLineArg.VERIFY_SSL), proxies=({"http": use_proxy, "https": use_proxy} if use_proxy else None), headers=base_headers)
